%% main_gun_detection_FLF_NB.m
% Gun detection with Fractional-order Legendre Functions (FLF) - Naive Bayes
...
fprintf('Fractional Legendre Features for Weapon Detection - Naive Bayes\n');
fprintf('================================================================\n');
...
fprintf('Training Naive Bayes classifier with FLF features...\n');

% Memory usage before training
mem_before = memory;

tic;
% Train Naive Bayes with Gaussian distribution
...
tic;
predicted_labels = predict(nb_model, X_test);
test_time = toc;
...
fprintf('\n=== FRACTIONAL LEGENDRE NAIVE BAYES RESULTS ===\n');
...
save('gun_detection_model_FLF_NB.mat', 'nb_model', 'n', 'alpha_scales', 'image_size', ...
     'train_idx', 'test_idx', 'accuracy', 'precision', 'recall', 'f1', ...
     'feature_extraction_time', 'training_time', 'time_per_image', 'memory_usage_MB');
fprintf('\nModel saved to gun_detection_model_FLF_NB.mat\n');

%% Plot confusion matrix
figure('Position', [100, 100, 600, 500]);
confusionchart(cm, {'No Gun', 'Gun'});
title(sprintf('FLF-Naive Bayes Confusion Matrix - Accuracy: %.2f%%', accuracy*100));